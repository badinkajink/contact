...
07; week 6 -> 17, 21; then the lab. `index.html` should carry this reading path.
...
| file | role | status |
|---|---|---|
| `deck.js` | navigation, build steps, present / read / print modes, KaTeX macros | exists; planned: `#id` hashes, `slidehidden` event, `Deck.pending(promise)` + `data-settled`, auto-loading `series.js` / `codemap.js` / `code.js`, series menu |
...
| `mj.js` | MuJoCo wrapper: `MJ.ready()`, `MJ.sim(xml)`, contacts with forces, `MJ.draw2` / `MJ.draw3` renderers into a `Fig`, `MJ.player` that runs only while its slide is shown | planned |
| `num.js` | small dense linear algebra (LU, QR, Cholesky, symmetric eigen, SVD, null / row space bases with rows orthonormal as in Hou & Mason, pinv, cond, RREF with a step log), seeded RNG, LP (simplex), QP, KKT solve, NNLS, finite differences | planned |
| `plot.js` | axes, curves, contours, heat maps, feasible regions, quiver, linear-map grids, 3D surfaces, histograms, strip charts | planned |
| `code.js`, `codemap.js` | slide-to-code overlay (section 7); `codemap.js` is generated | planned |
| `series.js` | the deck list for navigation and the series menu | planned |
| `models.js` | `tutorial/models/*.xml` as `window.MODELS`; generated | planned |
...
* A slide names that function with `data-code="NOTEBOOK.py:function_name"` on the `<section>` or
  on an inline element. `lib/code.js` (planned) turns it into a `[code]` link that opens the
...
|---|---|---|
| `00_math_toolkit.py` | 01-04 | planned |
...
| `04_friction_cones.py` | 11-13 | exists; in-text references still use the old deck numbers 01-03 |
| `05_contact_solvers.py` | 17-20 (tex ch. 7-8) | exists |
| `06_force_closure.py` | 14-16 | exists; in-text references still use the old deck numbers 04-06 |
| `07_hybrid_servoing.py` | 21 and the lab | planned |
| `08_contact_trajopt.py` | 22 | planned |
...
| `uv run tools/sync.py [--check]` | regenerates `lib/codemap.js` and `lib/models.js`; `--check` fails on stale files or unresolved `data-code` (planned) |
...
* Done: decks 11-16 (renumbered); the test tools (`check.py`, `shoot.py`, `twins.py`,
  `run_notebook.sh`); the vendored MuJoCo 3.14.0 WASM build and `tools/vendor_mujoco.py`;
  WASM feasibility (MuJoCo steps from `file://` at about 9 us per step).
* In progress, in this order: shared libraries (`mj.js`, `num.js`, `plot.js`, the `deck.js`
  upgrade, `code.js`, `series.js`, `sync.py`), notebook skeletons and deck header cells;
  slide-by-slide briefs for decks 01-10 and 17-22, the lab, and the integration of 11-16; the
  decks themselves; adversarial review of each deck (numbers, level, layout, style); the lab;
  corrections to the tex and cross-references from it to the decks; a rewritten `index.html`
  with the Hou & Mason reading path.
