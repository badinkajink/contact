...
tutorial/
  ...
  mjview.py                         mjviser 3D viewer inside a marimo notebook (show(model, data) -> iframe)
  slides/
...
| `mj.js` | MuJoCo wrapper: `MJ.ready()`, `MJ.sim(xml)`, contacts with forces, `MJ.draw2` / `MJ.draw3` renderers into a `Fig`, `MJ.player` that runs only while its slide is shown | reviewed 2026-09-24: contact forces are the force of geom1 on geom2 in the contact frame (resting 1 kg box: four contacts of 2.4525 N, 9.81 N up on the box), pyramidal and elliptic; the 18 cases in `tools/twins/mujoco.json` pass, and its Python-vs-WASM parity cases (box drop, resting contacts, tilted block, tumbling box, pinch) also pass at tolerance 0; no heap growth over 20000 player frames; `dispose()` no longer frees `m.opt`, an interior pointer; print mode entered by Ctrl+P now draws the `printSteps` frame; `tools/demos/mj_demo.html` passes `check.py` |
| `num.js` | small dense linear algebra (LU, QR, Cholesky, symmetric eigen, SVD, null / row space bases with rows orthonormal as in Hou & Mason, pinv, cond, RREF with a step log), seeded RNG, LP (simplex), QP, KKT solve, NNLS, finite differences | reviewed 2026-09-24: 173 twin cases in `tools/twins/num.json` pass, 60 of them new: rank-deficient, wide, tall, zero and 1e±200-scaled matrices through svd, the four subspaces, pinv and lstsq; cond of nearly parallel rows against exact Fraction arithmetic (relative error about eps·cond, as numpy's); eqQP with dependent and inconsistent rows; QPs at a 5-row apex, with duplicated rows, infeasible, or with a feasible unconstrained minimum; Kuhn's and Chvátal's cycling LPs, free-variable and equality-only LPs; 24 random degenerate LPs and 24 QPs built identically on both sides from `Num.rng` (`tools/twins/num_gen.js` and its port in `num_ref.py`), checked against HiGHS and active-set enumeration plus KKT residuals; NNLS whose least-squares solution is all negative; rng stream independence; mvnormal statistics of 20000 draws (3-D and singular covariance). Fixed: svd, qr and eigSym gave wrong factors for entries above about 1e77 or below 1e-77 (they now factor A / 2^e); `rng.sphere(0)` looped forever; mvnormal with a singular covariance put draws 6e-8 off its range; the qp convexity test was absolute, so 1e-12 diag(1, -1) returned a saddle as 'optimal'; `tex(Infinity)` gave "Infinity" and `tex(1.5e-7)` "1.5e-7"; qp, eqQP and mvnormal did not check shapes; a matrix with no columns crashed instead of raising a named error. In headless Chrome 149: svd 6x6 0.007 ms, 40x40 0.5 ms; qp with 6 variables 0.07-0.2 ms, with 40 variables and 40 rows 23 ms; a twin case keeps each 6x6 call under 1 ms |
| `plot.js` | axes, curves, contours, heat maps, feasible regions, quiver, linear-map grids, 3D surfaces, histograms, strip charts | reviewed 2026-09-24: `F.curve` bisects every interval whose chord misses the curve by more than 0.75 px (depth 10) and breaks the line at poles and jumps (1/x off the sample grid, tan x, floor x; `jumps: true` connects them); reversed limits (`xlim: [2, -2]`) draw right to left; automatic ticks use the 1-2-5 step nearest one tick per 90 px (x) or 55 px (y), with m×10^e labels past 10^6 and below 10^-3 (they were all "0" on [0, 1e-9]); a constant heat map takes the middle color (it was the bottom color, dark blue on the diverging map); ±Infinity is ordered in contours and takes the end colors in heat maps; `F.bars` moves the x label below its category labels; "-" inside `$...$` is a true minus; x labels sit 3 px higher, which cleared the 7 `check.py` warnings; a slider redraw of a 60×60 heat map with 10 labelled contour levels costs 1.9 ms mean and 5.2 ms max, the region demo 2.2 ms mean; `tools/demos/plot_demo.html` passes `check.py` with 0 warnings |
...
in a static deck. It lives in marimo notebooks through `tutorial/mjview.py`: `mjview.show(model,
data)` starts a viser server on a free local port with `mjviser.scene.ViserMujocoScene(server,
model, num_envs=1)` (which does not block) and returns a view whose `.iframe` points at
`http://localhost:PORT`; `.run(step_fn, seconds)` steps in a background thread and `.stop()`
ends it. mjviser is not a notebook dependency (run `uv run --with mujoco==3.14.0 --with mjviser
marimo edit NOTEBOOK.py`); without it `show()` returns a message. `05_contact_solvers.py` has the
example, behind a switch that scripted runs leave off.
...
(68 definitions, 3 models). The warnings on decks 11-16 are the missing slide ids and notes that
...
before building decks on them. `lib/mj.js`, `lib/plot.js`, `lib/num.js`, `tools/demos/mj_demo.html`,
`tools/demos/plot_demo.html`, `tools/twins/mujoco.json`, `tools/twins/num.json` and
`tutorial/mjview.py` were reviewed or written on 2026-09-24 (section 6). Known gaps:

* MuJoCo's soft contact makes the sliding block of `tilted_plane.xml` rock and hop: at 30
  degrees over 2 s its lowest corner moves between 0.6 mm inside and 1.3 mm above the ramp and
  46 % of the steps (pyramidal cone) have no contact, while the slip after 1 s (329.3 mm) is
  within 0.2 % of Coulomb's 328.6 mm. Deck 19 should measure how `solref`, the timestep and
  `condim` change it before using the model to teach sliding.
* The pinched box of `two_finger_pinch.xml` creeps down at 4.8 mm/s (pyramidal) and 2.6 mm/s
  (elliptic) with 5 N per pad, 2.5 times the Coulomb minimum; the model comment says so.
* `MJ.draw3` draws contact dots, arrows and cones after all geoms, so a contact behind a box is
  drawn over its front faces (visible in `mj_demo.html#tumbling-box` at azimuth 30 degrees).
* `mjview.py` was tested with mjviser 0.0.14, viser 1.1.1 and mujoco 3.14.0 (server start, a
  `curl` of its page, a background run, stop); its iframe has not been viewed inside a running
  `marimo edit` session.
* `plot.js` has no log-scale axes, which the convergence plots of decks 05 and 09 need. Until a
  `ylog` option exists, plot `Math.log10(y)` and pass `yticks` as `[k, "10$^{k}$"]` pairs.
* `F.param` samples uniformly and joins every pair of finite samples, so a parametric curve
  through a pole draws a spurious segment; only `F.curve` refines and breaks at poles.
* In box style the rotated y label overlaps the tick labels when `margin.l` is under about 45 px
  plus the widest tick label, and the two corner tick labels touch when both are wide (`−1×10⁻⁴`
  on both axes). `check.py` does not detect overlapping SVG text; look at the PNG.
* `F.heat` with an `xlim` / `ylim` larger than the plot box is not clipped to the box, and
  `F.contour` skips cells with a NaN corner, so its lines stop up to one cell short of a NaN
  region.
* `F.surface3` paints faces in order of the depth of their centres, which can misorder faces of
  a strongly non-convex surface (not seen on the demo's bowl and saddle).
* `Num.qp` with 40 variables and 40 rows takes 23 ms (39 active-set steps, each an SVD null-space
  basis of the working set and an SVD solve of the reduced Hessian), too slow for every frame at
  60 fps. A QR null space and a Cholesky solve with the SVD as fallback would cut the step cost;
  the last timing case of `num.json` measures it.
* `Num.qp` has no anti-cycling rule for degenerate working sets. None of the 24 generated
  degenerate QPs in `num.json`, nor 400 more in a scratch run against enumeration, cycled; a cycle
  would end as `'iteration_limit'` after 50 (n + m) + 100 steps.
* `Num.lp` and `Num.qp` compare pivots, reduced costs and multipliers with an absolute 1e-9, and
  no twin case has rows far from unit scale. Add cases with rows scaled by 1e-8 and 1e8 against
  HiGHS before a deck feeds them unnormalized rows (Hou & Mason normalize theirs).
