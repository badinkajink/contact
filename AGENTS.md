...
| `num.js` | small dense linear algebra (LU, QR, Cholesky, symmetric eigen, SVD, null / row space bases with rows orthonormal as in Hou & Mason, pinv, cond, RREF with a step log), seeded RNG, LP (simplex), QP, KKT solve, NNLS, finite differences | exists, not yet reviewed; 110 of 113 twin cases in `tools/twins/num.json` pass (section 10 lists the three failures) |
...
before building decks on them. Known problems:

* `tools/twins/num.json`: 3 of 113 cases fail. Two LP cases ("lp with an equality row and mixed
  bounds", "lp with a >= row written as a negative <= row") return a different optimal `x` than
  scipy while status and objective agree; the problems likely have several optima, so the cases
  should compare the objective (or make the optimum unique). "rref rank equals rank() on the
  integer matrices" gives rank 3 where numpy gives 2 on the fourth matrix: check `Num.rref`'s
  pivot tolerance.
