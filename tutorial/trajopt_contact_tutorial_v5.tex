...
1 & 1.150 & 1.609 & $-2.117$ & 1.256\\
2 & 1.256 & 1.435 & $-1.124$ & 1.312\\
3 & 1.312 & 1.390 & $-0.462$ & 1.335\\
\bottomrule
\end{tabular}
\end{center}
...
\textbf{Norms.}
The $p$-norm of $\x\in\R^{n}$, for $p\ge 1$ (for $p<1$ the triangle inequality fails):
\begin{equation}
\|\x\|_{p}=\Bigl(\sum_{i=1}^{n}|x_{i}|^{p}\Bigr)^{1/p}.
\end{equation}
Common cases: $p=2$ (Euclidean), $p=1$ (Manhattan), and the limit $p\to\infty$, $\|\x\|_{\infty}=\max_{i}|x_{i}|$.
...
change \emph{in the metric of the inertia matrix}: half of it,
$\tfrac12\|\bm{v}-\bm{v}^{f}\|_{\M}^{2}$, is the \emph{kinetic energy} of the velocity change. MuJoCo's
documentation calls this formulation Gauss's principle of least constraint.
